# Simple Performance Test for PocketFence Kernel (ASCII version)
# ============================================================
...
try {
    ...
    Write-Host "[SUCCESS] Kernel is ready!" -ForegroundColor Green
}
catch {
    Write-Host "[ERROR] Kernel is not responding. Start with: dotnet run --kernel" -ForegroundColor Red
    exit 1
}
...
Write-Host "  [SUCCESS] Success Rate: $successCount/10" -ForegroundColor Green
Write-Host "  [TIMING] Average Time: $([math]::Round($avgTime, 2))ms" -ForegroundColor White
Write-Host ""
...
Write-Host "  [SUCCESS] Success Rate: $healthSuccess/5" -ForegroundColor Green
Write-Host "  [TIMING] Average Time: $([math]::Round($avgHealthTime, 2))ms" -ForegroundColor White
Write-Host ""
...
Write-Host "  [SUCCESS] Success Rate: $statsSuccess/5" -ForegroundColor Green
Write-Host "  [TIMING] Average Time: $([math]::Round($avgStatsTime, 2))ms" -ForegroundColor White
Write-Host ""
...
Write-Host "  [SUCCESS] Success Rate: $contentSuccess/8" -ForegroundColor Green
Write-Host "  [TIMING] Average Time: $([math]::Round($avgContentTime, 2))ms" -ForegroundColor White
Write-Host ""
...
Write-Host "  [SUCCESS] Successful: $rapidSuccess/20" -ForegroundColor Green
Write-Host "  [BLOCKED] Rate Limited: $rapidBlocked/20 (expected behavior)" -ForegroundColor Yellow
if ($rapidTimes.Count -gt 0) {
    $avgRapidTime = ($rapidTimes | Measure-Object -Average).Average
    Write-Host "  [TIMING] Avg Response Time: $([math]::Round($avgRapidTime, 2))ms" -ForegroundColor White
}
Write-Host ""
...
Write-Host "  [API] URL Filtering:     $successCount/10     Avg: $([math]::Round($avgTime, 1))ms" -ForegroundColor White
Write-Host "  [API] Health Checks:     $healthSuccess/5      Avg: $([math]::Round($avgHealthTime, 1))ms" -ForegroundColor White
Write-Host "  [API] Statistics:        $statsSuccess/5      Avg: $([math]::Round($avgStatsTime, 1))ms" -ForegroundColor White
Write-Host "  [API] Content Analysis:  $contentSuccess/8      Avg: $([math]::Round($avgContentTime, 1))ms" -ForegroundColor White
Write-Host "  [SEC] Rate Limit Test:   $rapidSuccess/20     (Rate limiting active)" -ForegroundColor Yellow
...
if ($overallAvg -lt 100) {
    ...
    Write-Host "Performance Rating: ACCEPTABLE" -ForegroundColor Yellow
    Write-Host "   The kernel is functional but could be optimized." -ForegroundColor Yellow
}
